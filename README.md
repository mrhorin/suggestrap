...
|element|Object|The Object that has 3 Element objects. `target` key has the target form element that you specified. `suggestrap` key has an `<ul>` element that is generated by suggestrap to show suggestions. `style` key has a `<style>` element that is generated by suggestrap and applied to suggestion elements.|
|jsonUrl|string|A JSON URL with wildcards replaced by queries.|
|suggestions|string|Suggestions matching with a value that an user typed into the target form element when you passed values to values key in the first argument.|
...
|hasUrl()|boolean|Returns true if you passed URL to url key or values key in the first argument, else false.|
|hasValues()|boolean|Returns true if you passed values to values key in the first argument, else false.|
